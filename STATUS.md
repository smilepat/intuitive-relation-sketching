...
## 🎯 한 줄 상태
M4(콘텐츠 확장) 완료 — 다중 지문 라이브러리 + 학습 기록 히스토리 뷰 추가. 타입검사·빌드 통과. 브라우저 수동 검증 대기.
...
- [ ] 브라우저에서 M1~M4 기능 수동 확인  ← 현재 위치
- [ ] GitHub Pages 활성화 (Settings → Pages → Source: GitHub Actions)

## ⏭️ 다음에 할 일 (Next Actions)
1. `npm run dev`로 실행: 지문 3종 전환, 내 지문 입력, 학습 기록 저장→열기 확인
2. 리포지토리 Settings에서 Pages를 GitHub Actions로 활성화
3. 다음 방향 결정 (M5 접근성 / 품질: 테스트+CI / 콘텐츠 추가 등)

## 🤔 결정 대기 (Decisions Needed)
- 다음 마일스톤 방향: M5(접근성) vs 품질(Vitest 테스트·CI 강화) vs 지문 콘텐츠 추가
